...
<script>
    $(function () {
        ...
        $('#ok_btn').click(function () {
            var $oldPWD = $('#old_password'), $newPWD = $('#new_password'), $rePWD = $('#confirmed_password');
            if($oldPWD.val() == ''){
                layer.msg('原密码不能为空', {time: 1200});
                $oldPWD.focus();
                return false;
            }else if($oldPWD.val().length < 6 || $oldPWD.val().length > 18){
                layer.msg('原密码长度在6-18位之间', {time: 1200});
                $oldPWD.focus();
                return false;
            }else if($newPWD.val() == ''){
